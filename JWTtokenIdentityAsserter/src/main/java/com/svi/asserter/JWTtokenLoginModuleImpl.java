 ...
 *
 * It is used in one of two modes: - authentication where it validates the
 * user's password then populates the subject with the user and the user's
 * groups. - identity assertion where it checks that the user exists, then
 * populates the subject with the user and the user's groups.
 *
 * The JWTtokenAuthenticationProviderImpl creates an options hash map that is
 * passed to this login module. It contains one entry, named "database", that is
 * an object that manages the user and group definitions. It optionally contains
 * another entry, named "IdentityAssertion", that puts the login module in
 * "identity assertion" mode (vs. the default which is "authenticadtion" mode).
 *
 * It uses the built in WLSUserImpl and WLSGroupImpl classes to populate the
 * subject with users and groups.
 *
 * @author Copyright (c) 2002 by BEA Systems. All Rights Reserved.
 */
final public class JWTtokenLoginModuleImpl implements LoginModule {
    ...
    public void
        ...
        logger.warning("JWTtokenLoginModuleImpl.initialize");
     ...
     * Attempt to login.
     *
     * If we're in authentication mode, extract the user name and password from
     * the callback handler. If the user exists and the password matches, then
     * populate the subject with the user and the user's group. Otherwise, the
     * login fails.
     *
     * If we're in identity assertion mode, extract the user name (only) from
     * the callback handler. If the user exists, then populate the subject with
     * the user and the user's groups. Otherwise, the login fails.
     *
    ...
    public boolean login() throws LoginException {
        ...
        logger.warning("JWTtokenLoginModuleImpl.login");
    ...
    public boolean commit() throws LoginException {
        ...
        logger.warning("JWTtokenLoginModule.commit");
        if (loginSucceeded) {
    ...
    public boolean abort() throws LoginException {
        ...
        logger.warning("JWTtokenLoginModule.abort");
    ...
    public boolean logout() throws LoginException {
        ...
        logger.warning("JWTtokenLoginModule.logout");
        return true;
    }
     ...
     * for the user name and password if in authentication mode. Returns one for
    ...
    private String getUserName(Callback[] callbacks) throws LoginException {
        ...
        logger.warning("\tCallBackToken\t= " + userName);
        return userName;
    }

    /**
     * Add the user's groups to the list of principals to be added to the
     * subject.
     *
    ...
    private void addGroupsForSubject(String userName) {
        ...
        String groupName = "jwtAuthenticated";
        logger.warning("\tgroupName\t= " + groupName);
        principalsForSubject.add(new WLSGroupImpl(groupName));
        if (callbackHandler instanceof JWTtokenCallbackHandlerImpl) {
            List<String> grpList = ((JWTtokenCallbackHandlerImpl) callbackHandler).getGroups();
            Iterator<String> it = grpList.iterator();
            while (it.hasNext()) {
                groupName = it.next();
                principalsForSubject.add(new WLSGroupImpl(groupName));
                logger.warning("\tgroupName\t= " + groupName);
     ...
     * assertion mode.
     *
     * @param useName A String containing the name of the user (already
     * retrieved from the callbacks). Only passed in so that we can print a
    ...
    private String getPasswordHave(String userName, Callback[] callbacks) throws LoginException {
        ...
        logger.warning("\tpasswordHave\t= " + passwd);
        return passwd;
    }
}
